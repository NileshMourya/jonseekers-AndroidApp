...
import { deleteBookMarks } from '../db/model';
const { width } = Dimensions.get('window');

const JobsCard = ({ items, setRefetching }) => {
  const navigation = useNavigation();

  const handleBookmark = async details => {
    try {
      setRefetching(true);
      await deleteBookMarks(details);
    } catch (error) {
      console.log(error);
    } finally {
      setRefetching(false);
    ...
    >
      <View style={styles.cardHeader}>
        ...
        >
          ...
          <TouchableOpacity onPress={() => handleBookmark(items.id)}>
            <Ionicons name={`trash-sharp`} color="black" size={20}></Ionicons>
          </TouchableOpacity>
        </View>
